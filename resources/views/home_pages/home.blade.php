<div id="__next">
    <div style="position:fixed;bottom:16px;right:16px;z-index:70000"></div>
    <div id="id" class="relative flex h-screen flex-col justify-between">
        <div>
            <x-header />
            <div class="h-[84px] w-full"></div>
            <main class="container relative mx-auto">
                <div class="flex items-center p-[10px]"><svg class="mr-[10px]" viewBox="0 0 1024 1024"
                        style="display: inline-block; stroke: currentcolor; fill: currentcolor; width: 30px; height: 30px;">
                        <path
                            d="M975.006 512c0 255.711-207.295 463.006-463.006 463.006s-463.006-207.295-463.006-463.006c0-255.711 207.295-463.006 463.006-463.006s463.006 207.295 463.006 463.006z"
                            fill="rgb(253, 187, 24)"></path>
                        <path
                            d="M512 0c-282.281 0-512 229.719-512 512s229.719 512 512 512 512-229.719 512-512-229.719-512-512-512zM102.205 511.676c32.771-40.882 73.653-76.897 117.455-107.721 0.649 120.051 26.606 239.452 80.791 342.631 3.245 5.84 6.489 11.681 9.734 17.196-41.856 1.947-83.387 1.947-124.918 0-52.887-74.302-80.791-162.88-83.062-252.106zM922.768 524.654c-0.324 20.766-1.947 41.531-5.191 62.297-31.148 23.361-63.919 43.802-97.987 61.648 14.601-115.833-1.298-234.586-52.563-348.147 0-0.324-0.324-0.649-0.324-0.973 36.989 7.463 73.977 17.521 110.317 29.526 28.553 60.025 45.1 126.54 45.749 195.65zM417.582 755.346c-45.749-71.057-77.222-151.199-91.823-238.155-9.409-56.132-11.681-113.561-8.112-170.667 106.748-51.589 220.958-69.759 335.169-62.621 7.138 13.627 13.952 27.579 20.117 41.207 53.861 119.726 68.786 243.995 48.994 365.019-89.551 32.446-184.619 51.589-278.388 62.621-8.76 0.973-17.521 1.947-25.957 2.596zM368.913 98.961c0-0.324 0-0.649 0.324-0.973 40.558-14.601 83.387-22.712 127.838-22.063 45.425 41.207 85.333 90.525 118.753 143.412-78.844 0.973-157.688 12.33-234.261 37.313-19.143 6.165-37.962 13.627-56.781 21.090 5.516-38.611 13.303-77.222 23.361-114.535 7.138-26.93 10.707-40.233 20.766-64.243zM728.416 224.527c-23.037-40.882-49.967-80.142-80.142-116.157 68.786 29.202 127.513 74.951 172.938 131.731-30.499-6.489-61.648-12.005-92.796-15.574zM253.080 165.8c-15.25 52.238-25.308 106.423-30.175 161.582-38.935 22.388-75.924 48.345-110.641 76.897 9.734-44.451 25.957-87.929 48.994-128.162 24.659-43.153 56.132-80.142 91.823-110.317zM345.227 818.94c32.446 45.425 70.408 85.982 112.264 122.646-82.089-16.872-159.635-59.701-220.309-117.779 36.34-0.324 72.355-1.947 108.046-4.867zM598.956 943.858c-0.973-0.973-1.947-1.947-3.245-2.92-53.861-38.611-101.232-83.387-140.816-133.354 84.684-11.032 170.342-28.553 253.404-54.51-16.872 62.946-42.829 124.269-77.871 183.321-10.383 3.245-20.766 5.516-31.473 7.463zM747.883 878.317c26.281-51.914 46.398-105.774 59.052-160.608 30.499-12.978 60.025-28.228 88.578-44.451-28.553 80.791-79.169 154.119-147.63 205.060z"
                            fill="rgb(40, 56, 75)"></path>
                    </svg>
                    <div class="flex items-center justify-between">
                        <div class="flex">
                            <div class="flex">
                                <h3 class="text-xl font-semibold" style=" margin-left: 10px; font-size: 1.15em;">
                                    រុករកតាមប្រភេទ</h3>
                                <h3 class="pl-[5px] text-xl font-bold text-primary"></h3>
                            </div>
                            <div></div>
                        </div>
                    </div>
                </div>

                {{-- ++++++++++++++++++++++++++++++++++ --}}

                <div class="relative w-full">
                    <div class="w-full overflow-hidden">
                        <div style="transform: translate3d(0px, 0px, 0px);"
                            class="relative flex h-auto w-auto select-none py-[20px]">
                            {{-- ======================================================= --}}
                            <div class="group flex flex-col items-center justify-center">
                                <div class="jomnot-fix-safari-radius relative ml-[10px] h-[120px] w-[190px] flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700 sm:w-[201px] md:w-[180px] lg:w-[193px] xl:w-[202px] 2xl:w-[208px]"
                                    style="background-color: rgb(54, 174, 30);">
                                    <a>
                                        <div class="absolute h-full w-full rounded-[16px]">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F9098218a-46e3-42a1-8193-02e6afdc5000%2Fpublic&w=3840&q=80"
                                                    class="object-cover brightness-100 transition-all duration-700 ease-in-out group-hover:scale-125 group-hover:brightness-75"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;">
                                            </span>
                                        </div>
                                    </a>
                                </div>
                            </div>
                            <div class="group flex flex-col items-center justify-center">
                                <div class="jomnot-fix-safari-radius relative ml-[10px] h-[120px] w-[190px] flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700 sm:w-[201px] md:w-[180px] lg:w-[193px] xl:w-[202px] 2xl:w-[208px]"
                                    style="background-color: rgb(54, 174, 30);">
                                    <a>
                                        <div class="absolute h-full w-full rounded-[16px]">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F9e50ccab-1068-4c78-ef3c-d85e39c16900%2Fpublic&w=3840&q=80"
                                                    class="object-cover brightness-100 transition-all duration-700 ease-in-out group-hover:scale-125 group-hover:brightness-75"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;">
                                            </span>
                                        </div>
                                    </a>
                                </div>
                            </div>
                            <div class="group flex flex-col items-center justify-center">
                                <div class="jomnot-fix-safari-radius relative ml-[10px] h-[120px] w-[190px] flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700 sm:w-[201px] md:w-[180px] lg:w-[193px] xl:w-[202px] 2xl:w-[208px]"
                                    style="background-color: rgb(54, 174, 30);">
                                    <a>
                                        <div class="absolute h-full w-full rounded-[16px]">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F68c1bd5b-e8d1-4e1b-fb16-666840c12700%2Fpublic&w=3840&q=80"
                                                    class="object-cover brightness-100 transition-all duration-700 ease-in-out group-hover:scale-125 group-hover:brightness-75"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;">
                                            </span>
                                        </div>
                                    </a>
                                </div>
                            </div>
                            <div class="group flex flex-col items-center justify-center">
                                <div class="jomnot-fix-safari-radius relative ml-[10px] h-[120px] w-[190px] flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700 sm:w-[201px] md:w-[180px] lg:w-[193px] xl:w-[202px] 2xl:w-[208px]"
                                    style="background-color: rgb(54, 174, 30);">
                                    <a>
                                        <div class="absolute h-full w-full rounded-[16px]">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F736da743-439b-490d-5818-34c54bf51000%2Fpublic&w=3840&q=80"
                                                    class="object-cover brightness-100 transition-all duration-700 ease-in-out group-hover:scale-125 group-hover:brightness-75"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;">
                                            </span>
                                        </div>
                                    </a>
                                </div>
                            </div>
                            <div class="group flex flex-col items-center justify-center">
                                <div class="jomnot-fix-safari-radius relative ml-[10px] h-[120px] w-[190px] flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700 sm:w-[201px] md:w-[180px] lg:w-[193px] xl:w-[202px] 2xl:w-[208px]"
                                    style="background-color: rgb(54, 174, 30);">
                                    <a>
                                        <div class="absolute h-full w-full rounded-[16px]">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F82cb3df2-ab9c-4b84-dde1-ae91b1039300%2Fpublic&w=3840&q=80"
                                                    class="object-cover brightness-100 transition-all duration-700 ease-in-out group-hover:scale-125 group-hover:brightness-75"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;">
                                            </span>
                                        </div>
                                    </a>
                                </div>
                            </div>
                            <div class="group flex flex-col items-center justify-center">
                                <div class="jomnot-fix-safari-radius relative ml-[10px] h-[120px] w-[190px] flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700 sm:w-[201px] md:w-[180px] lg:w-[193px] xl:w-[202px] 2xl:w-[208px]"
                                    style="background-color: rgb(54, 174, 30);">
                                    <a>
                                        <div class="absolute h-full w-full rounded-[16px]">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fcd11ace7-a580-49b5-7336-178d4bac5400%2Fpublic&w=3840&q=80"
                                                    class="object-cover brightness-100 transition-all duration-700 ease-in-out group-hover:scale-125 group-hover:brightness-75"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;">
                                            </span>
                                        </div>
                                    </a>
                                </div>
                            </div>
                            <div class="group flex flex-col items-center justify-center">
                                <div class="jomnot-fix-safari-radius relative ml-[10px] h-[120px] w-[190px] flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700 sm:w-[201px] md:w-[180px] lg:w-[193px] xl:w-[202px] 2xl:w-[208px]"
                                    style="background-color: rgb(54, 174, 30);">
                                    <a>
                                        <div class="absolute h-full w-full rounded-[16px]">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F17e71c85-dd25-4a2e-945b-de8c15a26f00%2Fpublic&w=3840&q=80"
                                                    class="object-cover brightness-100 transition-all duration-700 ease-in-out group-hover:scale-125 group-hover:brightness-75"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;">
                                            </span>
                                        </div>
                                    </a>
                                </div>
                            </div>
                            {{-- ======================================================= --}}



                            {{-- ======================================================= --}}



                        </div>
                    </div>
                </div>

                {{-- ++++++++++++++++++++++++++++++++++ --}}

                <div class="h-[12px] w-full"></div>

                {{-- ++++++++++++++++++++++++++++++++++ --}}

                <div class="flex items-center p-[10px]"><svg class="mr-[8px]" viewBox="0 0 776 1024"
                        style="display: inline-block; stroke: currentcolor; fill: currentcolor; width: 35px; height: 35px;">
                        <path
                            d="M704.595 222.089c-26.64-4.349-53.823-7.883-80.463-11.689-20.388-2.99-44.309-2.99-63.881-9.786-27.999-9.514-36.969-38.329-48.658-62.25-13.32-26.64-26.368-53.551-39.688-80.191-10.058-20.388-19.844-38.872-41.047-49.746-28.271-14.407-64.425-10.33-90.249 7.34-19.028 13.048-27.727 32.62-37.513 52.464-13.32 27.183-26.912 54.367-40.231 81.55-10.602 21.475-18.485 45.125-43.493 52.464-23.106 6.524-50.018 7.068-73.939 10.602-48.387 7.068-114.17 5.98-138.364 57.357-26.368 55.998 26.912 93.783 61.978 128.034 19.028 18.485 39.96 36.154 57.629 55.998 23.378 26.64 15.766 53.008 10.602 83.725-5.709 33.979-11.689 67.959-17.397 101.666-1.087 7.068-2.718 14.135-3.806 21.475v0c0 0.544 0 0.816 0 1.359-0.272 1.087-0.272 2.175-0.544 3.262 0 0.816 0 2.447 0 4.349-0.816 14.951 2.175 27.727 7.611 37.513 1.631 2.99 3.262 5.709 5.437 8.699 0 0 0 0 0 0 71.492 91.608 142.985 182.945 214.205 274.553 8.155 10.602 16.854 26.368 30.445 30.717 22.562 7.34 39.416-7.883 51.649-23.65 38.872-50.018 78.016-100.035 116.889-149.781 35.067-45.125 70.133-89.977 105.472-135.102 0 0 0.816-1.087 1.903-2.99 0 0 0-0.272 0.272-0.272 1.631-2.447 3.534-5.98 5.437-10.602 0.272-0.272 0.272-0.816 0.544-1.359 0.544-1.087 0.816-2.175 1.359-3.262 0 0 0-0.272 0-0.272 4.078-10.602 7.611-24.465 4.893-37.785-1.903-14.407-4.893-28.814-7.34-43.222-6.252-36.969-14.679-74.483-19.028-111.724-4.893-39.688 22.29-58.716 47.843-83.453 23.921-23.378 47.843-46.756 71.764-70.133 45.94-51.105 12.776-115.53-50.289-125.859z"
                            fill="rgb(40, 56, 75)"></path>
                        <path
                            d="M431.401 130.752l36.698 112.539c5.98 18.485 23.378 31.261 42.95 31.261h118.52c43.765 0 61.978 55.998 26.64 81.55l-95.686 69.59c-15.766 11.417-22.562 31.805-16.31 50.561l36.698 112.539c13.592 41.591-33.979 76.114-69.318 50.561l-95.686-69.59c-15.766-11.417-37.241-11.417-53.008 0l-95.686 69.59c-35.338 25.824-82.909-8.971-69.318-50.561l36.698-112.539c5.98-18.485-0.544-38.872-16.31-50.561l-97.045-69.59c-35.338-25.824-17.126-81.55 26.64-81.55h118.52c19.572 0 36.969-12.504 42.95-31.261l36.698-112.539c13.048-41.591 71.764-41.591 85.356 0z"
                            fill="rgb(253, 187, 24)"></path>
                    </svg>
                    <div class="flex items-center justify-between">
                        <div class="flex">
                            <div class="flex">
                                <h3 class="text-xl font-semibold" style="margin-left: 10px; font-size: 1.15rem;">
                                    រុករកតាមខេត្តក្រុង</h3>
                                <h3 class="pl-[5px] text-xl font-bold text-primary"></h3>
                            </div>
                            <div></div>
                        </div>
                    </div>
                </div>

                {{-- ====================================== --}}

                <div
                    class="relative grid h-auto w-full select-none flex-wrap gap-2 px-[10px] py-[20px] sm:grid-cols-2 sm:px-[0px] md:grid md:grid-cols-3 xl:grid xl:grid-cols-5">
                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/category">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="សៀមរាប"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F2ea6bb4f-b2f3-47ad-4d6f-6beeea58a500%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">សៀមរាប</span>
                                </div>
                            </a>
                        </div>
                    </div>

                    {{-- ============================== --}}



                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/cate">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="កំពត"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F086f61a7-6b22-4ef7-f9ad-fe07f3654300%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">កំពត</span>
                                </div>
                            </a>
                        </div>
                    </div>

                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a>
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="កែប"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F26735f54-1c0e-4171-4509-8b38299d0700%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">កែប</span>
                                </div>
                            </a>
                        </div>
                    </div>

                    {{-- ============================== --}}

                    {{-- 1 --}}



                    {{-- 2 --}}
                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/kep">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="បាត់ដំបង"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fdfbde414-2886-40d1-f875-dd8a24a50c00%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">បាត់ដំបង </span>
                                </div>
                            </a>
                        </div>
                    </div>

                    {{-- 3 --}}


                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/battambang">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="ភ្នំពេញ"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F9f40c6c4-6af9-4546-33d7-099fb56b7a00%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">ភ្នំពេញ</span>
                                </div>
                            </a>
                        </div>
                    </div>

                    {{-- 4 --}}

                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/vietnam">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="កំពង់ស្ពឺ"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F2719f4ae-ca4b-4202-5114-574d85949c00%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">កំពង់ស្ពឺ</span>
                                </div>
                            </a>
                        </div>
                    </div>


                    {{-- 5 --}}

                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/vietnam">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="កោះកុង"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fe614baa4-7d35-486f-2f2e-f178b1c56c00%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">កោះកុង</span>
                                </div>
                            </a>
                        </div>
                    </div>

                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/vietnam">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="ភ្នំ​​ ១៥០០"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F25752d6b-9cbb-43e2-846f-a392c8a0ee00%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">ភ្នំ​​ ១៥០០</span>
                                </div>
                            </a>
                        </div>
                    </div>


                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/vietnam">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="មណ្ឌលគិរី"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F78d8f2ae-8914-4a57-2ee6-1ca42e554900%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">មណ្ឌលគិរី</span>
                                </div>
                            </a>
                        </div>
                    </div>





                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/vietnam">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="វៀតណាម"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F5ac1760a-b138-45f2-ff6d-08e3ed473600%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">វៀតណាម</span>
                                </div>
                            </a>
                        </div>
                    </div>

                    {{-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ --}}


                    {{-- <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/koh-kong">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="កោះកុង"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F2719f4ae-ca4b-4202-5114-574d85949c00%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">កោះកុង</span>
                                </div>
                            </a>
                        </div>
                    </div>
                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/phnom-1500">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="ភ្នំ 1500"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fe614baa4-7d35-486f-2f2e-f178b1c56c00%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">ភ្នំ 1500</span>
                                </div>
                            </a>
                        </div>
                    </div>
                    <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/mondulkiri">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="មណ្ឌលគិរី"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F25752d6b-9cbb-43e2-846f-a392c8a0ee00%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">មណ្ឌលគិរី</span>
                                </div>
                            </a>
                        </div>
                    </div> --}}

                    {{-- ========== --}}


                    {{-- ========== --}}



                    {{-- <div class="group flex flex-col items-center justify-center">
                        <div
                            class="jomnot-fix-safari-radius relative h-[150px] w-full flex-shrink-0 cursor-pointer overflow-hidden rounded-[16px] shadow-base duration-700">
                            <a href="/km/destinations/vietnam">
                                <div class="absolute h-full w-full rounded-[16px]"><span
                                        style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;"><img
                                            alt="វៀតណាម"
                                            src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F5ac1760a-b138-45f2-ff6d-08e3ed473600%2Fpublic&w=3840&q=80"
                                            class="h-full w-full rounded-[16px] object-cover transition-all duration-700 ease-in-out group-hover:scale-125"
                                            style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"
                                            sizes="100vw"><noscript></noscript></span>
                                </div>
                                <div
                                    class="absolute flex h-full w-full items-center justify-center bg-secondary bg-opacity-30 text-white">
                                    <span class="text-2xl font-semibold">វៀតណាម</span>
                                </div>
                            </a>
                        </div>
                    </div> --}}
                </div>

                <div class="h-[12px] w-full"></div>

                {{-- ==================================================================== --}}

                <div class="p-[10px]">
                    <a
                        href="/km/search?searchValue=&amp;page=1&amp;limit=10&amp;model=Article&amp;location=phnom_penh&amp;locale=km">
                        <div class="flex items-center"><svg class="mr-[10px]" viewBox="0 0 950 1024"
                                style="display: inline-block; stroke: currentcolor; fill: currentcolor; width: 30px; height: 30px;">
                                <path
                                    d="M318.159 288.364l54.454-220.896c8.562-34.59 53.084-44.522 75.344-16.439l142.127 177.744c8.904 10.959 22.603 17.124 36.645 16.096l227.061-16.439c35.617-2.74 58.563 36.645 39.042 66.44l-125.003 190.074c-7.877 11.987-9.247 26.713-3.767 39.727l85.961 210.965c13.357 32.878-16.781 67.125-51.371 57.878l-219.526-60.276c-13.699-3.767-28.425-0.685-39.042 8.562l-173.977 146.922c-27.398 22.946-69.18 4.795-70.892-30.823l-10.617-227.403c-0.685-14.041-8.219-27.056-20.206-34.59l-193.498-120.209c-30.138-18.836-26.028-64.385 7.534-76.714l213.019-80.482c13.357-5.137 23.288-16.096 26.713-30.138z"
                                    fill="rgb(253, 187, 24)"></path>
                                <path
                                    d="M950.025 947.628c0-1.37 0.342-2.74 0-3.767 0 0 0-0.342 0-0.342 0-0.685-0.685-1.027-0.685-1.712-0.342-1.027-0.685-2.397-1.37-3.082-11.302-23.288-63.358-27.056-82.879-30.138-66.098-9.932-133.223-13.357-200.005-15.411-148.634-5.137-304.46-7.877-451.724 16.781-31.85 5.48-89.728 12.329-107.537 44.522-18.151 33.563 38.7 43.152 59.933 47.262 142.812 27.398 295.898 23.631 440.423 19.864 76.372-2.055 153.771-4.795 229.116-16.781 28.768-4.795 95.551-12.672 111.304-45.892 1.37-2.397 2.74-4.795 3.082-7.534 0 0 0-0.342 0.342-0.342 0 0 0 0 0 0 0-1.37 0-2.397 0-3.425z"
                                    fill="rgb(40, 56, 75)"></path>
                                <path
                                    d="M857.557 425.354c27.056-41.439 59.248-82.194 82.879-126.031 19.179-35.96 8.219-76.714-28.083-96.235-23.631-12.672-50.001-10.274-75.687-8.219-60.618 4.452-121.236 8.904-181.854 13.357-4.11 0.342-14.384 2.74-19.864 2.055 1.712-1.712-5.822-11.987-4.795-10.274-17.124-25.001-38.7-48.289-57.536-71.92-32.535-40.755-72.947-116.441-128.771-126.373-47.262-8.562-101.030 15.069-118.496 60.961-7.534 19.521-10.959 41.439-15.754 61.645-13.699 55.823-27.398 111.304-41.097 167.128-0.685 0-1.712 0-4.11 1.712-7.192 4.795-18.151 6.849-26.028 9.932-31.508 11.644-62.673 23.288-94.181 35.275-45.207 17.124-115.414 30.823-137.332 79.454-27.74 61.645 35.96 91.783 78.084 118.154 27.398 17.124 54.796 34.247 82.536 51.029 17.809 10.959 42.124 21.233 57.193 35.617 11.987 11.302 10.274 19.179 10.959 34.247 1.37 31.85 3.082 63.358 4.452 95.208 2.055 41.439-8.219 118.839 20.548 153.429 48.974 59.248 123.633 15.754 168.84-21.918 23.973-20.206 47.946-40.412 71.92-60.618 19.521-16.439 47.604-33.22 63.358-53.426 26.028 10.274 56.166 15.069 83.221 22.603 47.946 13.014 106.167 39.385 157.196 37.33 42.467-1.712 92.126-30.823 97.948-76.714 2.74-22.261-8.219-42.467-16.439-62.33-22.603-55.481-46.919-110.962-68.153-167.128-5.48-14.384-8.562-21.576-1.712-36.987 9.932-20.891 28.083-41.439 40.755-60.961zM346.585 860.982c5.48-4.795 2.055 3.425 0 0v0zM776.391 395.559c-20.548 31.165-60.961 73.975-65.413 113.359-4.452 39.042 27.74 89.043 41.782 123.291 10.959 26.371 21.576 53.084 32.535 79.454 2.055 5.137 7.877 28.425 13.014 41.439-1.027-0.342-2.397-0.342-3.425-0.685-29.795-8.219-59.248-16.096-89.043-24.316-41.097-11.302-89.043-32.193-132.195-30.823-52.741 1.712-90.413 44.179-128.086 75.687-21.576 18.151-43.494 36.645-65.070 54.796-3.082 2.74-22.603 15.754-34.932 26.371 0-0.342 0-0.685 0-1.027 0.342-5.822-0.685-11.644-0.685-17.124-1.027-25.001-2.397-50.344-3.425-75.344-2.055-44.522-2.74-89.728-6.165-134.25-4.452-56.166-65.413-79.112-107.537-105.14-27.74-17.124-55.481-34.59-83.564-51.714-11.644-7.192-36.645-17.124-41.439-31.165-6.165-17.466 14.041-17.466 29.795-23.288 30.138-11.302 60.276-22.603 90.071-33.905 48.974-18.494 122.606-32.535 143.155-86.989 0.685-1.712 0.685-3.082 1.37-4.795 0.685-1.37 1.37-2.74 1.712-4.452 10.617-43.494 21.576-86.989 32.193-130.483 5.137-20.206 9.932-40.412 15.069-60.618 9.247 10.617 21.233 22.261 25.343 27.398 18.494 23.288 37.33 46.577 55.823 69.522 31.165 39.042 57.193 86.646 112.674 89.386 42.467 1.37 86.989-6.165 129.798-9.247 14.726-1.027 29.11-2.055 43.837-3.082l43.152-4.11c0 0 0.342 0 0.342 0h1.712c8.219-0.685 40.070-1.027 14.726 33.22 0 0 0 0 0 0l-6.165 7.877c0 0 0 0 0 0-3.767 5.137-7.877 10.274-10.274 13.699-16.781 25.686-33.563 51.371-50.686 77.057z"
                                    fill="rgb(40, 56, 75)"></path>
                            </svg>
                            <div class="w-full">
                                <div class="flex items-center justify-between">
                                    <div class="flex">
                                        <div class="flex">
                                            <h3 class="text-xl font-semibold" style="margin-left: 10px;">ចំណត</h3>
                                            <h3 class="pl-[5px] text-xl font-bold text-primary">ភ្នំពេញ</h3>
                                        </div>
                                        <div></div>
                                    </div><span class="text-xs font-semibold text-primary"><a href="/article"
                                            class="text-xs font-semibold text-primary"> មើលទាំងអស់</a></span>
                                </div>
                            </div>
                        </div>
                    </a>
                </div>

                {{-- =================================================================== --}}

                <div class="relative w-full">
                    <div class="w-full overflow-hidden" style="overflow: auto;">
                        <div class="relative flex h-auto w-full select-none py-[20px]"
                            style="transform: translate3d(0px, 0px, 0px);">

                            {{-- 1 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Ff33f4ca6-d98a-480f-7eca-77de80e5c100%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 2 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F99fe0ed2-513f-4f59-c609-cac7f399f700%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 3 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F7496d072-4cb5-433b-f984-c0a6adde6400%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 4 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fa99bf7e9-e859-495a-6214-e91c71d9ca00%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 5 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fbdcc27b0-1dcd-408d-0acf-a5bb575cb500%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 6 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fe65b51dd-2cb9-4b34-db86-9857d29fff00%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 7 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F294e858d-e6b0-40ac-2660-66835f0e5200%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 8 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F57c79a35-39c1-4745-dfad-02f518b36200%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 9 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F5270fcd2-de2e-4a6b-8825-bf65f5f48800%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 10 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F8473c2a3-489e-4e91-3419-08bc13fdc700%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 11 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F8473c2a3-489e-4e91-3419-08bc13fdc700%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 12 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F925047bf-4157-4fd5-6e10-158758302300%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 13 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F47def856-15a2-46a8-ffd1-3815fa221800%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 14 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F00c59cd5-ddf2-4927-7e7c-502bd4824400%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 15 --}}




                        </div>
                    </div>



                    {{-- ===================================================== --}}
                    <button
                        class="shadow-lg hover:shadow-xl flex absolute top-1/2 right-2.5 z-10 ml-4 h-8 w-8  -translate-y-1/2 scale-100 cursor-pointer touch-manipulation items-center justify-center rounded-full border-0 bg-white fill-gray-900 p-2 outline-0 transition-transform hover:scale-110 hover:fill-yellow-500 disabled:hidden">
                        <svg class="h-full w-full" viewBox="0 0 238.003 238.003">
                            <path
                                d="M181.776 107.719L78.705 4.648c-6.198-6.198-16.273-6.198-22.47 0s-6.198 16.273 0 22.47l91.883 91.883-91.883 91.883c-6.198 6.198-6.198 16.273 0 22.47s16.273 6.198 22.47 0l103.071-103.039a15.741 15.741 0 0 0 4.64-11.283c0-4.13-1.526-8.199-4.64-11.313z">
                            </path>
                        </svg>
                    </button>
                </div>

                {{-- saperate div --}}

                <div class="h-[12px] w-full"></div>

                {{-- again --}}


                <div class="p-[10px]">
                    <a
                        href="/km/search?searchValue=&amp;page=1&amp;limit=10&amp;model=Article&amp;location=phnom_penh&amp;locale=km">
                        <div class="flex items-center"><svg class="mr-[10px]" viewBox="0 0 950 1024"
                                style="display: inline-block; stroke: currentcolor; fill: currentcolor; width: 30px; height: 30px;">
                                <path
                                    d="M318.159 288.364l54.454-220.896c8.562-34.59 53.084-44.522 75.344-16.439l142.127 177.744c8.904 10.959 22.603 17.124 36.645 16.096l227.061-16.439c35.617-2.74 58.563 36.645 39.042 66.44l-125.003 190.074c-7.877 11.987-9.247 26.713-3.767 39.727l85.961 210.965c13.357 32.878-16.781 67.125-51.371 57.878l-219.526-60.276c-13.699-3.767-28.425-0.685-39.042 8.562l-173.977 146.922c-27.398 22.946-69.18 4.795-70.892-30.823l-10.617-227.403c-0.685-14.041-8.219-27.056-20.206-34.59l-193.498-120.209c-30.138-18.836-26.028-64.385 7.534-76.714l213.019-80.482c13.357-5.137 23.288-16.096 26.713-30.138z"
                                    fill="rgb(253, 187, 24)"></path>
                                <path
                                    d="M950.025 947.628c0-1.37 0.342-2.74 0-3.767 0 0 0-0.342 0-0.342 0-0.685-0.685-1.027-0.685-1.712-0.342-1.027-0.685-2.397-1.37-3.082-11.302-23.288-63.358-27.056-82.879-30.138-66.098-9.932-133.223-13.357-200.005-15.411-148.634-5.137-304.46-7.877-451.724 16.781-31.85 5.48-89.728 12.329-107.537 44.522-18.151 33.563 38.7 43.152 59.933 47.262 142.812 27.398 295.898 23.631 440.423 19.864 76.372-2.055 153.771-4.795 229.116-16.781 28.768-4.795 95.551-12.672 111.304-45.892 1.37-2.397 2.74-4.795 3.082-7.534 0 0 0-0.342 0.342-0.342 0 0 0 0 0 0 0-1.37 0-2.397 0-3.425z"
                                    fill="rgb(40, 56, 75)"></path>
                                <path
                                    d="M857.557 425.354c27.056-41.439 59.248-82.194 82.879-126.031 19.179-35.96 8.219-76.714-28.083-96.235-23.631-12.672-50.001-10.274-75.687-8.219-60.618 4.452-121.236 8.904-181.854 13.357-4.11 0.342-14.384 2.74-19.864 2.055 1.712-1.712-5.822-11.987-4.795-10.274-17.124-25.001-38.7-48.289-57.536-71.92-32.535-40.755-72.947-116.441-128.771-126.373-47.262-8.562-101.030 15.069-118.496 60.961-7.534 19.521-10.959 41.439-15.754 61.645-13.699 55.823-27.398 111.304-41.097 167.128-0.685 0-1.712 0-4.11 1.712-7.192 4.795-18.151 6.849-26.028 9.932-31.508 11.644-62.673 23.288-94.181 35.275-45.207 17.124-115.414 30.823-137.332 79.454-27.74 61.645 35.96 91.783 78.084 118.154 27.398 17.124 54.796 34.247 82.536 51.029 17.809 10.959 42.124 21.233 57.193 35.617 11.987 11.302 10.274 19.179 10.959 34.247 1.37 31.85 3.082 63.358 4.452 95.208 2.055 41.439-8.219 118.839 20.548 153.429 48.974 59.248 123.633 15.754 168.84-21.918 23.973-20.206 47.946-40.412 71.92-60.618 19.521-16.439 47.604-33.22 63.358-53.426 26.028 10.274 56.166 15.069 83.221 22.603 47.946 13.014 106.167 39.385 157.196 37.33 42.467-1.712 92.126-30.823 97.948-76.714 2.74-22.261-8.219-42.467-16.439-62.33-22.603-55.481-46.919-110.962-68.153-167.128-5.48-14.384-8.562-21.576-1.712-36.987 9.932-20.891 28.083-41.439 40.755-60.961zM346.585 860.982c5.48-4.795 2.055 3.425 0 0v0zM776.391 395.559c-20.548 31.165-60.961 73.975-65.413 113.359-4.452 39.042 27.74 89.043 41.782 123.291 10.959 26.371 21.576 53.084 32.535 79.454 2.055 5.137 7.877 28.425 13.014 41.439-1.027-0.342-2.397-0.342-3.425-0.685-29.795-8.219-59.248-16.096-89.043-24.316-41.097-11.302-89.043-32.193-132.195-30.823-52.741 1.712-90.413 44.179-128.086 75.687-21.576 18.151-43.494 36.645-65.070 54.796-3.082 2.74-22.603 15.754-34.932 26.371 0-0.342 0-0.685 0-1.027 0.342-5.822-0.685-11.644-0.685-17.124-1.027-25.001-2.397-50.344-3.425-75.344-2.055-44.522-2.74-89.728-6.165-134.25-4.452-56.166-65.413-79.112-107.537-105.14-27.74-17.124-55.481-34.59-83.564-51.714-11.644-7.192-36.645-17.124-41.439-31.165-6.165-17.466 14.041-17.466 29.795-23.288 30.138-11.302 60.276-22.603 90.071-33.905 48.974-18.494 122.606-32.535 143.155-86.989 0.685-1.712 0.685-3.082 1.37-4.795 0.685-1.37 1.37-2.74 1.712-4.452 10.617-43.494 21.576-86.989 32.193-130.483 5.137-20.206 9.932-40.412 15.069-60.618 9.247 10.617 21.233 22.261 25.343 27.398 18.494 23.288 37.33 46.577 55.823 69.522 31.165 39.042 57.193 86.646 112.674 89.386 42.467 1.37 86.989-6.165 129.798-9.247 14.726-1.027 29.11-2.055 43.837-3.082l43.152-4.11c0 0 0.342 0 0.342 0h1.712c8.219-0.685 40.070-1.027 14.726 33.22 0 0 0 0 0 0l-6.165 7.877c0 0 0 0 0 0-3.767 5.137-7.877 10.274-10.274 13.699-16.781 25.686-33.563 51.371-50.686 77.057z"
                                    fill="rgb(40, 56, 75)"></path>
                            </svg>
                            <div class="w-full">
                                <div class="flex items-center justify-between">
                                    <div class="flex">
                                        <div class="flex">
                                            <h3 class="text-xl font-semibold" style="margin-left: 10px;">ចំណត</h3>
                                            <h3 class="pl-[5px] text-xl font-bold text-primary">សៀមរាប</h3>
                                        </div>
                                        <div></div>
                                    </div><span class="text-xs font-semibold text-primary"><a href="/article"
                                            class="text-xs font-semibold text-primary"> មើលទាំងអស់</a></span>
                                </div>
                            </div>
                        </div>
                    </a>
                </div>

                {{-- =================================================================== --}}

                <div class="relative w-full">
                    <div class="w-full overflow-hidden" style="overflow: auto;">
                        <div class="relative flex h-auto w-full select-none py-[20px]"
                            style="transform: translate3d(0px, 0px, 0px);">

                            {{-- 1 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Ff33f4ca6-d98a-480f-7eca-77de80e5c100%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 2 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F99fe0ed2-513f-4f59-c609-cac7f399f700%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 3 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F7496d072-4cb5-433b-f984-c0a6adde6400%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 4 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fa99bf7e9-e859-495a-6214-e91c71d9ca00%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 5 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fbdcc27b0-1dcd-408d-0acf-a5bb575cb500%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 6 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fe65b51dd-2cb9-4b34-db86-9857d29fff00%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 7 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F294e858d-e6b0-40ac-2660-66835f0e5200%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 8 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F57c79a35-39c1-4745-dfad-02f518b36200%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 9 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F5270fcd2-de2e-4a6b-8825-bf65f5f48800%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 10 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F8473c2a3-489e-4e91-3419-08bc13fdc700%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 11 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F8473c2a3-489e-4e91-3419-08bc13fdc700%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 12 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F925047bf-4157-4fd5-6e10-158758302300%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 13 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F47def856-15a2-46a8-ffd1-3815fa221800%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 14 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F00c59cd5-ddf2-4927-7e7c-502bd4824400%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 15 --}}




                        </div>
                    </div>



                    {{-- ===================================================== --}}
                    <button
                        class="shadow-lg hover:shadow-xl flex absolute top-1/2 right-2.5 z-10 ml-4 h-8 w-8  -translate-y-1/2 scale-100 cursor-pointer touch-manipulation items-center justify-center rounded-full border-0 bg-white fill-gray-900 p-2 outline-0 transition-transform hover:scale-110 hover:fill-yellow-500 disabled:hidden">
                        <svg class="h-full w-full" viewBox="0 0 238.003 238.003">
                            <path
                                d="M181.776 107.719L78.705 4.648c-6.198-6.198-16.273-6.198-22.47 0s-6.198 16.273 0 22.47l91.883 91.883-91.883 91.883c-6.198 6.198-6.198 16.273 0 22.47s16.273 6.198 22.47 0l103.071-103.039a15.741 15.741 0 0 0 4.64-11.283c0-4.13-1.526-8.199-4.64-11.313z">
                            </path>
                        </svg>
                    </button>
                </div>

                {{-- end --}}

                {{-- saperate div --}}
                <div class="h-[12px] w-full"></div>

                {{-- again --}}


                <div class="p-[10px]">
                    <a
                        href="/km/search?searchValue=&amp;page=1&amp;limit=10&amp;model=Article&amp;location=phnom_penh&amp;locale=km">
                        <div class="flex items-center"><svg class="mr-[10px]" viewBox="0 0 950 1024"
                                style="display: inline-block; stroke: currentcolor; fill: currentcolor; width: 30px; height: 30px;">
                                <path
                                    d="M318.159 288.364l54.454-220.896c8.562-34.59 53.084-44.522 75.344-16.439l142.127 177.744c8.904 10.959 22.603 17.124 36.645 16.096l227.061-16.439c35.617-2.74 58.563 36.645 39.042 66.44l-125.003 190.074c-7.877 11.987-9.247 26.713-3.767 39.727l85.961 210.965c13.357 32.878-16.781 67.125-51.371 57.878l-219.526-60.276c-13.699-3.767-28.425-0.685-39.042 8.562l-173.977 146.922c-27.398 22.946-69.18 4.795-70.892-30.823l-10.617-227.403c-0.685-14.041-8.219-27.056-20.206-34.59l-193.498-120.209c-30.138-18.836-26.028-64.385 7.534-76.714l213.019-80.482c13.357-5.137 23.288-16.096 26.713-30.138z"
                                    fill="rgb(253, 187, 24)"></path>
                                <path
                                    d="M950.025 947.628c0-1.37 0.342-2.74 0-3.767 0 0 0-0.342 0-0.342 0-0.685-0.685-1.027-0.685-1.712-0.342-1.027-0.685-2.397-1.37-3.082-11.302-23.288-63.358-27.056-82.879-30.138-66.098-9.932-133.223-13.357-200.005-15.411-148.634-5.137-304.46-7.877-451.724 16.781-31.85 5.48-89.728 12.329-107.537 44.522-18.151 33.563 38.7 43.152 59.933 47.262 142.812 27.398 295.898 23.631 440.423 19.864 76.372-2.055 153.771-4.795 229.116-16.781 28.768-4.795 95.551-12.672 111.304-45.892 1.37-2.397 2.74-4.795 3.082-7.534 0 0 0-0.342 0.342-0.342 0 0 0 0 0 0 0-1.37 0-2.397 0-3.425z"
                                    fill="rgb(40, 56, 75)"></path>
                                <path
                                    d="M857.557 425.354c27.056-41.439 59.248-82.194 82.879-126.031 19.179-35.96 8.219-76.714-28.083-96.235-23.631-12.672-50.001-10.274-75.687-8.219-60.618 4.452-121.236 8.904-181.854 13.357-4.11 0.342-14.384 2.74-19.864 2.055 1.712-1.712-5.822-11.987-4.795-10.274-17.124-25.001-38.7-48.289-57.536-71.92-32.535-40.755-72.947-116.441-128.771-126.373-47.262-8.562-101.030 15.069-118.496 60.961-7.534 19.521-10.959 41.439-15.754 61.645-13.699 55.823-27.398 111.304-41.097 167.128-0.685 0-1.712 0-4.11 1.712-7.192 4.795-18.151 6.849-26.028 9.932-31.508 11.644-62.673 23.288-94.181 35.275-45.207 17.124-115.414 30.823-137.332 79.454-27.74 61.645 35.96 91.783 78.084 118.154 27.398 17.124 54.796 34.247 82.536 51.029 17.809 10.959 42.124 21.233 57.193 35.617 11.987 11.302 10.274 19.179 10.959 34.247 1.37 31.85 3.082 63.358 4.452 95.208 2.055 41.439-8.219 118.839 20.548 153.429 48.974 59.248 123.633 15.754 168.84-21.918 23.973-20.206 47.946-40.412 71.92-60.618 19.521-16.439 47.604-33.22 63.358-53.426 26.028 10.274 56.166 15.069 83.221 22.603 47.946 13.014 106.167 39.385 157.196 37.33 42.467-1.712 92.126-30.823 97.948-76.714 2.74-22.261-8.219-42.467-16.439-62.33-22.603-55.481-46.919-110.962-68.153-167.128-5.48-14.384-8.562-21.576-1.712-36.987 9.932-20.891 28.083-41.439 40.755-60.961zM346.585 860.982c5.48-4.795 2.055 3.425 0 0v0zM776.391 395.559c-20.548 31.165-60.961 73.975-65.413 113.359-4.452 39.042 27.74 89.043 41.782 123.291 10.959 26.371 21.576 53.084 32.535 79.454 2.055 5.137 7.877 28.425 13.014 41.439-1.027-0.342-2.397-0.342-3.425-0.685-29.795-8.219-59.248-16.096-89.043-24.316-41.097-11.302-89.043-32.193-132.195-30.823-52.741 1.712-90.413 44.179-128.086 75.687-21.576 18.151-43.494 36.645-65.070 54.796-3.082 2.74-22.603 15.754-34.932 26.371 0-0.342 0-0.685 0-1.027 0.342-5.822-0.685-11.644-0.685-17.124-1.027-25.001-2.397-50.344-3.425-75.344-2.055-44.522-2.74-89.728-6.165-134.25-4.452-56.166-65.413-79.112-107.537-105.14-27.74-17.124-55.481-34.59-83.564-51.714-11.644-7.192-36.645-17.124-41.439-31.165-6.165-17.466 14.041-17.466 29.795-23.288 30.138-11.302 60.276-22.603 90.071-33.905 48.974-18.494 122.606-32.535 143.155-86.989 0.685-1.712 0.685-3.082 1.37-4.795 0.685-1.37 1.37-2.74 1.712-4.452 10.617-43.494 21.576-86.989 32.193-130.483 5.137-20.206 9.932-40.412 15.069-60.618 9.247 10.617 21.233 22.261 25.343 27.398 18.494 23.288 37.33 46.577 55.823 69.522 31.165 39.042 57.193 86.646 112.674 89.386 42.467 1.37 86.989-6.165 129.798-9.247 14.726-1.027 29.11-2.055 43.837-3.082l43.152-4.11c0 0 0.342 0 0.342 0h1.712c8.219-0.685 40.070-1.027 14.726 33.22 0 0 0 0 0 0l-6.165 7.877c0 0 0 0 0 0-3.767 5.137-7.877 10.274-10.274 13.699-16.781 25.686-33.563 51.371-50.686 77.057z"
                                    fill="rgb(40, 56, 75)"></path>
                            </svg>
                            <div class="w-full">
                                <div class="flex items-center justify-between">
                                    <div class="flex">
                                        <div class="flex">
                                            <h3 class="text-xl font-semibold" style="margin-left: 10px;">ចំណត</h3>
                                            <h3 class="pl-[5px] text-xl font-bold text-primary">កំពត</h3>
                                        </div>
                                        <div></div>
                                    </div><span class="text-xs font-semibold text-primary"><a href="/article"
                                            class="text-xs font-semibold text-primary"> មើលទាំងអស់</a></span>
                                </div>
                            </div>
                        </div>
                    </a>
                </div>

                {{-- =================================================================== --}}

                <div class="relative w-full">
                    <div class="w-full overflow-hidden" style="overflow: auto;">
                        <div class="relative flex h-auto w-full select-none py-[20px]"
                            style="transform: translate3d(0px, 0px, 0px);">

                            {{-- 1 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Ff33f4ca6-d98a-480f-7eca-77de80e5c100%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 2 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F99fe0ed2-513f-4f59-c609-cac7f399f700%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 3 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F7496d072-4cb5-433b-f984-c0a6adde6400%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 4 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fa99bf7e9-e859-495a-6214-e91c71d9ca00%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 5 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fbdcc27b0-1dcd-408d-0acf-a5bb575cb500%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 6 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2Fe65b51dd-2cb9-4b34-db86-9857d29fff00%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 7 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F294e858d-e6b0-40ac-2660-66835f0e5200%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 8 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F57c79a35-39c1-4745-dfad-02f518b36200%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 9 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F5270fcd2-de2e-4a6b-8825-bf65f5f48800%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 10 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F8473c2a3-489e-4e91-3419-08bc13fdc700%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 11 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F8473c2a3-489e-4e91-3419-08bc13fdc700%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 12 --}}

                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F925047bf-4157-4fd5-6e10-158758302300%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 13 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F47def856-15a2-46a8-ffd1-3815fa221800%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 14 --}}


                            <div
                                class="h-full w-[296px] flex-shrink-0 cursor-pointer px-[5px] sm:w-[296px] md:w-[244px] lg:w-[244px] xl:w-[244px] 2xl:w-[296px]">
                                <div class="relative z-20 h-[250px] w-full overflow-hidden rounded-[16px]">
                                    <a href="">
                                        <div
                                            class="aspect-w-1 aspect-h-1 absolute h-full w-full transform bg-cover bg-center transition-all duration-700 ease-in-out hover:scale-125">
                                            <span
                                                style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                                                <img src="https://jomnot.com/_next/image?url=https%3A%2F%2Fimagedelivery.net%2FjGl7otiKn8qlUhSSX-Fxow%2F00c59cd5-ddf2-4927-7e7c-502bd4824400%2Fpublic&w=3840&q=80"
                                                    class="z-10 h-[250px] w-full rounded-[16px] object-cover"
                                                    style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;"></span>
                                        </div>
                                    </a>
                                </div>
                                <div class="my-[8px] flex flex-col"><span
                                        class="flex items-center text-[14px] text-gray-400"><svg
                                            class="h-[16px] w-[16px] stroke-gray-400" viewBox="0 0 819 1024"
                                            style="display: inline-block; stroke: currentcolor; fill: currentcolor;">
                                            <path
                                                d="M409.625 204.775c-112.947 0-204.8 91.853-204.8 204.8s91.853 204.799 204.8 204.799c112.948 0 204.801-91.853 204.801-204.799s-91.853-204.8-204.801-204.8zM409.625 511.975c-56.474 0-102.4-45.926-102.4-102.4s45.926-102.4 102.4-102.4c56.474 0 102.401 45.926 102.401 102.4s-45.927 102.4-102.401 102.4zM409.42-0.025c-233.421 0-409.395 176.077-409.395 409.6 0 323.891 246.374 525.311 352.307 596.838 17.306 11.725 37.223 17.613 57.089 17.613 19.814 0 39.629-5.842 56.934-17.516 106.087-71.629 352.82-273.147 352.82-596.935 0.051-233.523-176.128-409.6-409.754-409.6zM409.677 921.574c-92.365-62.413-307.252-237.21-307.252-511.999 0-178.022 129.075-307.2 306.995-307.2 175.258 0 307.405 132.045 307.405 307.2 0 274.636-215.194 449.587-307.149 511.999z">
                                            </path>
                                        </svg>&nbsp;ភ្នំពេញ</span><a
                                        href="/km/articles/jomnot-this-summer-starbucks-will-introduce-2-new-summer-drinks"><span
                                            class="text-[18px] font-semibold line-clamp-2">ភេសជ្ជ:សម្រាប់រដូវក្តៅទាំង​
                                            2 ប្រភេទហ្នឹងបង្ហាញខ្លួននៅ Starbucks នារដូវក្តៅនេះ</span></a></div>
                            </div>

                            {{-- 15 --}}




                        </div>
                    </div>



                    {{-- ===================================================== --}}
                    <button
                        class="shadow-lg hover:shadow-xl flex absolute top-1/2 right-2.5 z-10 ml-4 h-8 w-8  -translate-y-1/2 scale-100 cursor-pointer touch-manipulation items-center justify-center rounded-full border-0 bg-white fill-gray-900 p-2 outline-0 transition-transform hover:scale-110 hover:fill-yellow-500 disabled:hidden">
                        <svg class="h-full w-full" viewBox="0 0 238.003 238.003">
                            <path
                                d="M181.776 107.719L78.705 4.648c-6.198-6.198-16.273-6.198-22.47 0s-6.198 16.273 0 22.47l91.883 91.883-91.883 91.883c-6.198 6.198-6.198 16.273 0 22.47s16.273 6.198 22.47 0l103.071-103.039a15.741 15.741 0 0 0 4.64-11.283c0-4.13-1.526-8.199-4.64-11.313z">
                            </path>
                        </svg>
                    </button>
                </div>

                {{-- end --}}






            </main>
        </div>
        <div>
            {{-- tver alov ai --}}
            <div class="h-[104px] w-full"></div>
            <div class="hidden lg:flex">
                <x-footer />
            </div>
            <div class="fixed bottom-[30px] right-[30px] z-20">
                <button>
                    <img src="" style="width: 46px; height: 46px; border-radius: 50%; background: black;"
                        class="h-[46px] w-[46px] rounded-full text-secondary duration-500 hover:text-primary hover:opacity-80">
                </button>
            </div>
        </div>
    </div>
</div>
